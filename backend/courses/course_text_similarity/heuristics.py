...
def title_heuristics(title_a, title_b):
    ...
    """
    title_a, title_b = title_a.strip().lower(), title_b.strip().lower()
    # Case 1
    ...
    level_a = levels.get(title_a.split()[0])
    level_b = levels.get(title_b.split()[0])
    if level_a != level_b:
        return True
    return False


def description_heuristics(desc1, desc2):
    """
