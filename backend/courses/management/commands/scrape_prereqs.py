...
def load_pairs_from_file(file_path: Path) -> list[tuple[str, str]]:
    ...
    if suffix == ".csv":
        ...
        with file_path.open(newline="") as handle:
            ...
            if "code" not in fieldnames or "crn" not in fieldnames:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "Defaults to timestamped file in courses/data/prereq_scrapes."
            ),
        )
        parser.add_argument(
            "--timeout-seconds",
    ...
    def handle(self, *args, **kwargs):
        ...
        if all_course_codes:
            ...
            self.stdout.write(
                f"Loaded {len(db_pairs)} course/CRN pairs from DB "
                f"for semesters: {', '.join(semesters)}"
            )
        ...
        if kwargs["output_file"]:
            ...
            output_path = DEFAULT_OUTPUT_DIR / f"prereq_scrape_{timestamp}.json"
        ...
        if sleep_seconds > 0 or jitter_seconds > 0:
            self.stdout.write(
                f"Throttling enabled: base sleep {sleep_seconds:.3f}s, "
                f"jitter up to {jitter_seconds:.3f}s"
            )
        ...
        self.stdout.write(
            f"Requests: {len(records)} | With clssnotes: {with_notes} | "
            f"Without clssnotes: {without_notes}"
        )
