...
def precompute_pcr_views(verbose=False, is_new_data=False):
    ...
    objs_to_update = []
    cache_deletes = set()
    same_count = has_count = total_count = 0
    ...
    with transaction.atomic():
            ...
            if topic_id in topic_set:
                ...
                if is_new_data:
                    ...
                    response_obj.response = manual_course_reviews(
                        course_code_list[0], topic.most_recent.semester
                    )
                ...
                try:
                    review_json = manual_course_reviews(course_code_list[0], topic.most_recent.semester)
        ...
        if verbose:
            print(f"{total_count} course reviews updated, {has_count} of which were already in the database. Only {total_count - same_count} course reviews were updated.")
