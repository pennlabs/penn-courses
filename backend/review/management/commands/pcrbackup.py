...
from review.import_reviews import (
    save_all_reviews,
    save_reviews_for_department,
    save_reviews_for_semester,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if kwargs["sem"] is not None:
            ...
            save_reviews_for_semester(
                kwargs["sem"], os.path.join(directory, f'{kwargs["sem"]}.json')
            )
        elif kwargs["dept"] is not None:
            print(f'fetching reviews for department {kwargs["dept"]}...')
            save_reviews_for_department(
                kwargs["dept"], os.path.join(directory, f'{kwargs["dept"]}.json')
            )
        else:
