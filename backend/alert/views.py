...
def accept_webhook(request):
    ...
    if username != settings.WEBHOOK_USERNAME or password != settings.WEBHOOK_PASSWORD:
        logger.error("Credentials could not be verified", extra={"request": request})
        return HttpResponse(
    ...
    if course_status is None:
        logger.error("Course Status couldn't be extracted from resp.", extra={"request": request})
    ...
    try:
        ...
        if course_term is None:
            logger.error("Course Term couldn't be extracted from resp.", extra={"request": request})
        ...
        if course_term.upper().endswith("B"):
            logger.error("webhook ignored (summer class)", extra={"request": request})
        ...
        if should_send_pca_alert(course_term, course_status):
            try:
                ...
                logger.error("course code could not be parsed", extra={"request": request})
