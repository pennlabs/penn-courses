...
input a dict mapping string paths to dicts, where each subdict maps string methods to dicts, and
...
custom_tag_descriptions = {
    ...
    "[PCA] Registration": dedent(
        ...
        push notifications to receive a close notification. Note that the close_notification
        setting carries over across resubscriptions, but can be disabled at any time using
        Update Registration.
                    ...
                    for response in ob["responses"]:
                        ...
                        if "content" not in data["paths"][path][method]["responses"][code].keys():
                            raise ValueError(
                                ...
                                f"If '[SCHEMA]' is not in the description for this "
                                f"path/method/code, no response schema content will be created for "
                                "it and you will not be able to make an example response for it. "
...
class PcxAutoSchema(AutoSchema):
    ...
    Once the code from this PR is included in a stable Django release, the get_operation methods
    below should be deleted (it will be inherited from AutoSchema) and the tag code should be
    ...
    def _get_responses(self, path, method):
        ...
        if method == "DELETE":
            if (
                ...
                or not self.response_codes[path][method]
            ):
                return {"204": {"description": ""}}
            return {
                str(code): {"description": description.replace("[SCHEMA]", "")}
                for code, description in self.response_codes[path][method].items()
            }
        ...
        if (
            ...
            # If custom response codes are not specified, use 200
            if path in self.override_schema and method in self.override_schema[path].keys():
                # If schema is overridden by override_schema
                return {
                    **{
                        "200": {
                            "description": "",
            ...
            # Schema is not overridden by override_schema
            return {
                **{
                    "200": {
                        "content": {
        ...
        # Custom response codes are specified (whether or not to include the auto-generated
        # response schema in the route is dependant on whether "[SCHEMA]" is included in the
        # description).
        if path in self.override_schema and method in self.override_schema[path].keys():
            # If schema is overridden by override_schema
            return {
                **{
                    str(code): {
                        "description": description.replace("[SCHEMA]", ""),
                        **(
                            {
                                "content": {
                                    content_type: {
