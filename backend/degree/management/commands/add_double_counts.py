...
class Command(BaseCommand):
    help = dedent(
        """
        Adds allowed double counts for every rule of every degree (note that we add double
        count allows to rule LEAVES.) To add double counts, do this command: python
        manage.py add_double_counts
        """
    )
    ...
    def handle(self, *args, **kwargs):
        ...
        - (Any leaf in this rule) -> (Double count allowed with Rule1),
            (Double count allowed with Rule2), ...
        ...
        if kwargs["test"]:
            foundations_rule = (
                ...
                .filter(degree="BA")[0]
                .rules.all()
                .filter(title="General Education, Foundations")[0]
            )
            ...
            sectors_rule = (
                ...
                .filter(degree="BA")[0]
                .rules.all()
                .filter(title="General Education, Sectors")[0]
            )
            ...
            major_rule = (
                ...
                .filter(degree="BA")[0]
                .rules.all()
                .filter(title__icontains="Major in History of Art")[0]
            )
            ...
            area_lists_rule = (
                Degree.objects.all()
                .filter(degree="BSE", major_name__icontains="Computer Science")[0]
                .rules.all()
                .filter(title__icontains="Major in Computer Science")[0]
                .children.all()
                .filter(title__icontains="Area Lists")[0]
            )
        ...
        for degree in Degree.objects.all():
            ...
            for entry in double_count_entries:
                if degree.degree == entry["type"] and (
                    entry["major_code"] == degree.major or entry["major_code"] == "ALL_MAJORS"
                ):
                    home_rule = next(
                        (
                            rule
                            for rule in rule_children_dict.keys()
                            if re.match(entry["home_rule"], rule.title)
                        ),
                        None,
                    )
                    if home_rule:
                            ...
                            for allow_double_count in entry["allow_double_count"]:
                                if allow_double_count == "ALL_RULES":
                                    for double_rule in degree.rules.all():
                                        rule.can_double_count_with.add(
                                            *rule_children_dict[double_rule]
                                        )
                                        rule.save()
                                else:
                                    double_rule = next(
                                        (
                                            rule
                                            for rule in rule_children_dict.keys()
                                            if re.match(allow_double_count, rule.title)
                                        ),
                                        None,
                                    )
                                    if double_rule:
