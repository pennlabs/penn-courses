...
class BulkTestCases(TestCase):
    def add_courses(self):
        ...
        for rec in cis_recs:
            Section.objects.get(full_code="CIS-120-001").associated_sections.add(
                Section.objects.get(full_code=rec)
            )
            Section.objects.get(full_code="CIS-120-002").associated_sections.add(
                Section.objects.get(full_code=rec)
            )
        for lab in phys_labs_1:
            Section.objects.get(full_code="PHYS-150-001").associated_sections.add(
                Section.objects.get(full_code=lab)
            )
        for lab in phys_labs_2:
            Section.objects.get(full_code="PHYS-150-002").associated_sections.add(
                Section.objects.get(full_code=lab)
            )
        for rec in cis_recs:
            for lec in cis_lecs:
