...
from tests.courses.util import create_mock_data, create_data_with_type

from alert import tasks
from alert.models import SOURCE_PCA, Registration, RegStatus, register_for_course, RegistrationGroup
from courses.models import StatusUpdate, Section, Course
from courses.util import get_or_create_course_and_section, get_or_create_course
...
#
#
class BulkTestCases(TestCase):
    def add_courses(self):
        create_data_with_type("CIS-120-001", TEST_SEMESTER, "LEC")
        create_data_with_type("CIS-120-002", TEST_SEMESTER, "LEC")

        create_data_with_type("CIS-120-202", TEST_SEMESTER, "REC")
        create_data_with_type("CIS-120-203", TEST_SEMESTER, "REC")
        create_data_with_type("CIS-120-204", TEST_SEMESTER, "REC")

        cis_recs = ["CIS-120-203", "CIS-120-204", "CIS-120-202"]

        create_data_with_type("PHYS-150-001", TEST_SEMESTER, "LEC")
        create_data_with_type("PHYS-150-002", TEST_SEMESTER, "LEC")

        create_data_with_type("PHYS-150-121", TEST_SEMESTER, "LAB")
        create_data_with_type("PHYS-150-123", TEST_SEMESTER, "LAB")
        create_data_with_type("PHYS-150-124", TEST_SEMESTER, "LAB")
        create_data_with_type("PHYS-150-125", TEST_SEMESTER, "LAB")

        cis_lecs = ["CIS-120-001", "CIS-120-002"]
        phys_lecs = ["PHYS-150-001", "PHYS-150-002"]
        phys_labs_1 = ["PHYS-150-121", "PHYS-150-123", "PHYS-150-124"]
        phys_labs_2 = ["PHYS-150-125"]

        for rec in cis_recs:
            Section.objects.get(full_code="CIS-120-001").associated_sections.add(Section.objects.get(full_code=rec))
            Section.objects.get(full_code="CIS-120-002").associated_sections.add(Section.objects.get(full_code=rec))
        for lab in phys_labs_1:
            Section.objects.get(full_code="PHYS-150-001").associated_sections.add(Section.objects.get(full_code=lab))
        for lab in phys_labs_2:
            Section.objects.get(full_code="PHYS-150-002").associated_sections.add(Section.objects.get(full_code=lab))
        for rec in cis_recs:
            for lec in cis_lecs:
                Section.objects.get(full_code=rec).associated_sections.add(
                    Section.objects.get(full_code=lec)
                )
        for lab in phys_labs_1 + phys_labs_2:
            for lec in phys_lecs:
                Section.objects.get(full_code=lab).associated_sections.add(
                    Section.objects.get(full_code=lec)
                )
    ...
    def test_bulk_register_lec_with_lec(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["LEC"], "section": "CIS-120-001"}),
            content_type="application/json",
        )
    ...
    def test_bulk_register_lec_with_rec(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["LEC"], "section": "CIS-120-202"}),
            content_type="application/json",
        )
    ...
    def test_bulk_register_rec_lab_lec_with_lec(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["REC", "LEC", "LEC"], "section": "CIS-120-001"}),
            content_type="application/json",
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(Registration.objects.all()), 5)
        self.assertEqual(len(RegistrationGroup.objects.all()), 1)
        for reg in all_regs:
            self.assertEqual(reg.bulk_registration, all_bulk[0])

    def test_bulk_register_rec_lab_lec_with_rec(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["REC", "LAB", "LEC"], "section": "CIS-120-202"}),
            content_type="application/json",
        )
    ...
    def test_bulk_register_specific_lab_with_lec(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["LAB"], "section": "PHYS-150-001"}),
            content_type="application/json",
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(Registration.objects.all()), 3)
        self.assertEqual(len(RegistrationGroup.objects.all()), 1)
        for reg in all_regs:
            self.assertEqual(reg.bulk_registration, all_bulk[0])

    def test_bulk_register_all_lec_with_specific_lab(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["LEC"], "section": "PHYS-150-121"}),
            content_type="application/json",
        )
    ...
    def test_bulk_register_specific_lab_with_lab(self):
        response = self.client.post(
            reverse("registrations-bulk"),
            json.dumps({"type": ["LAB"], "section": "PHYS-150-125"}),
            content_type="application/json",
        )
        ...
        # self.assertContains list comprehension ([r.section.full_code for r in Registration.objects.all()] )
        # && cis_recs
...
class MultiTestCases(TestCase):
    ...
    def test_multi_empty(self):
        ...
        self.assertEqual(len(Registration.objects.all()), 0)
        self.assertEqual(len(RegistrationGroup.objects.all()), 0)
    ...
    def test_multi_many(self):
        courses = ["CIS-120-001", "CIS-120-002", "CIS-120-202"]
        response = self.client.post(
            reverse("registrations-multi"),
            json.dumps({"courses": courses}),
            content_type="application/json",
    ...
    def test_multi_many_auto_resubscribe(self):
        courses = ["CIS-120-001", "CIS-120-002", "CIS-120-202"]
        response = self.client.post(
            reverse("registrations-multi"),
            json.dumps({"courses": courses, "auto_resubscribe": True}),
            content_type="application/json",
        )
        ...
        for reg in all_regs:
            self.assertEqual(reg.bulk_registration, all_bulk[0])
            self.assertTrue(reg.auto_resubscribe, True)
        for course in courses:
            self.assertIn(course, all_secs)

    def test_multi_all_bad(self):
        response = self.client.post(
            reverse("registrations-multi"),
            json.dumps({"courses": ["abcde", "fghi", "qijfwe"]}),
            content_type="application/json",
        )
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(len(all_bulk), 0)
        self.assertEqual(len(all_regs), 0)

    def test_multi_first_bad_rest_good(self):
        response = self.client.post(
            reverse("registrations-multi"),
            json.dumps({"courses": ["abcdefg", "CIS-120-002", "CIS-120-202"]}),
            content_type="application/json",
        )
    ...
    def test_multi_first_good_rest_bad(self):
        courses = ["CIS-120-001", "abcd", "efgh"]
        ...
        self.assertIn(actual_course[0], all_secs)
