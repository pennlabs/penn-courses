...
class Review(models.Model):
    ...
    Actual scores for the review is stored in the ReviewBit related object, can be accessed via
    the `reviewbit_set` of the object.
    """
    ...
    def get_averages(course_code, instructor_name=None, fields=None):
        ...
        return qs.aggregate(
            **{
                # average the score of all the reviewbits of a certain field (that's where the
                # filter comes in)
...
class ReviewBit(models.Model):
    """
    A single key/value pair associated with a review. Fields are things like "course_quality", and
    scores are averages which range from 0 to 4.
    """
