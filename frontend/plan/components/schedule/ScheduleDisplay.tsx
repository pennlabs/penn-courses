...
const ScheduleDisplay = ({
    ...
    if (ref.current) {
      ...
      requestAnimationFrame(() =>
        takeScreenshot(element, {
          // Use the actual element dimensions
          width: rect.width,
          height: rect.height,
          // Alternative: you can also try scrollHeight/scrollWidth for full content
  ...
  const getUncrampedImage = () => {
    if (ref.current) {
      const element = ref.current;
      const scheduleContents = element.querySelector('[class*="ScheduleContents"]');
      ...
      const originalStyles = {
        ...
        contents: scheduleContents ? {
          height: scheduleContents.style.height,
          minHeight: scheduleContents.style.minHeight,
          maxHeight: scheduleContents.style.maxHeight,
        } : null
      };

      // Remove height constraints temporarily
      element.style.height = 'auto';
      element.style.minHeight = 'auto';
      element.style.maxHeight = 'none';

      if (scheduleContents) {
        scheduleContents.style.height = 'auto';
        scheduleContents.style.minHeight = 'auto';
        scheduleContents.style.maxHeight = 'none';
      }

      // Wait for layout to settle
      requestAnimationFrame(() => {
        setTimeout(() => {
          takeScreenshot(element).then((image) => {
            // Restore original styles
            element.style.height = originalStyles.element.height;
            element.style.minHeight = originalStyles.element.minHeight;
            element.style.maxHeight = originalStyles.element.maxHeight;

            if (scheduleContents && originalStyles.contents) {
              scheduleContents.style.height = originalStyles.contents.height;
              scheduleContents.style.minHeight = originalStyles.contents.minHeight;
              scheduleContents.style.maxHeight = originalStyles.contents.maxHeight;
            }

            download(image);
          });
        }, 200); // Give more time for layout
      });
    }
  };
  ...
  return (
    <ScheduleBox>
      <button onClick={getUncrampedImage}>Hello</button>
      <ScheduleContents $dims={dims} $notEmpty={notEmpty} ref={ref}>
