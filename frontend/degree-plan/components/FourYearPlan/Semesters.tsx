...
const ModifySemesters = ({
    ...
    const latestSemester =
        Object.keys(semesters)
            ...
            .pop() || "2026A"; // TODO: Change fallback value to start semester (based on onboarding?)
    ...
    return (
        ...
        <AddSemesterContainer className={className}>
            ...
            <Select
                ...
                onChange={(option) =>
                    setSelectedSeason(option ? option.value : selectedSeason)
                }
            />
...
export const interpolateSemesters = (
    startingYear: number,
...
interface SemestersContextProps {
    semesterRefs: React.MutableRefObject<{
        [semester: string]: HTMLDivElement | null;
    }>;
}
...
const Semesters = ({
    ...
    const getDefaultSemesters = useCallback(() => {
        const startingYear = currentSemester
            ? Number(currentSemester.substring(0, 4))
            : new Date().getFullYear(); // Use current semester as default starting semester
    ...
    const removeSemester = (semester: string) => {
        ...
        if (semesters[semester]) {
            ...
            for (var sem in semesters) {
                if (sem !== semester)
                    newSems = { ...newSems, [sem]: semesters[sem] };
            }
    ...
    useEffect(() => {
        ...
        if (typeof window === "undefined")
            return setSemesters(getDefaultSemesters());
