...
const translateSemester = (semester: Course["semester"]) => {
    if (semester === TRANSFER_CREDIT_SEMESTER_KEY)
        return "AP & Transfer Credit";
    const year = semester.slice(0, 4);
    const term = semester.slice(4);
    return `${
        term === "A" ? "Spring" : term === "B" ? "Summer" : "Fall"
    } ${year}`;
};
...
export const SkeletonSemester = ({ showStats }: { showStats: boolean }) => {
    return (
        <SemesterCard
            $isDroppable={false}
            $isOver={false}
            $semesterComparison={1}
        >
            <SemesterHeader>
                <SemesterLabel>
                    <Skeleton width="5em" />
...
const FlexSemester = ({
    ...
    const credits = fulfillments.reduce(
        (acc, curr) => acc + (curr.course?.credits || 1),
        0,
    );

    const { createOrUpdate: addToDock } = useSWRCrud<DockedCourse>(
        `/api/degree/docked`,
        { idKey: "full_code" },
    );
    ...
    >(
        () => ({
            accept: [
                ...
                ItemTypes.COURSE_IN_REQ,
                ItemTypes.COURSE_IN_SEARCH,
            ],
            drop: (course: DnDCourse) => {
                if (course.rule_id === undefined || course.rule_id == null) {
                    // moved from plan or dock
                    createOrUpdate({ semester }, course.full_code);
                } else {
                    // moved from req panel
                    fetch(
                        `/api/degree/satisfied-rule-list/${activeDegreeplanId}/${course.full_code}/${course.rule_id}`,
                    ).then((r) => {
                        r.json().then((data) => {
                            const selectedRules = data["selected_rules"].reduce(
                                (res: any, obj: any) => {
                                    res.push(obj.id);
                                    return res;
                                },
                            ...
                            const unselectedRules = data[
                                "unselected_rules"
                            ].reduce((res: any, obj: any) => {
                                res.push(obj.id);
                                return res;
                            }, []);
                            ...
                            for (let obj of data["new_selected_rules"]) {
                                if (obj.id != course.rule_id) {
                                    showToast(
                                        `${course.full_code} also fulfilled ${obj.title}!`,
                                        false,
                                    );
                                }
                            }
    ...
    return (
        <SemesterCard
            ...
            ref={drop}
            $semesterComparison={
                currentSemester ? semester.localeCompare(currentSemester) : 1
            }
        >
            <SemesterHeader>
                <SemesterLabel>{translateSemester(semester)}</SemesterLabel>
            ...
            <CreditsLabel>{credits} CUs</CreditsLabel>
        </SemesterCard>
    );
};
