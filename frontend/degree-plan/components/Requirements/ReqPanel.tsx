...
export const WhiteSpace = styled.div<{ $headerHeight: number; $zIndex?: number }>`
    ...
    z-index: ${(props) => props.$zIndex ?? 991};
`;
...
const DegreeHeader = ({
    ...
    const degreeName = !skeleton ? (
        `${degree.degree} in ${degree.major_name} ${
            ...
            <DegreeTitleWrapper>
                ...
                <DegreeYear>
                    {!skeleton ? (
                        ...
                        <DarkBlueBackgroundSkeleton width="4em" />
                    )}
                </DegreeYear>
            </DegreeTitleWrapper>
            <span>
...
const computeRuleTree = ({
    ...
    if (rule.q) {
        // Rule leaf
        const fulfillmentsForRule: Fulfillment[] =
            rulesToFulfillments[rule.id] || [];
        const unselectedFulfillmentsForRule: Fulfillment[] =
            rulesToUnselectedFulfillments[rule.id] || [];
        const cus = fulfillmentsForRule.reduce(
            (acc, f) => acc + (f.course?.credits || 1),
            0,
        ); // default to 1 cu
        const num = fulfillmentsForRule.length;
        const progress = Math.min(
            rule.credits ? cus / rule.credits : 1,
            rule.num ? num / rule.num : 1,
        );
        return {
            activeDegreePlanId,
            type: "LEAF",
    ...
    const progress =
        children.reduce((acc, { progress }) => progress + acc, 0) /
        Math.min(children.length, rule.num || Infinity);
    return {
        num: rule.num || undefined,
        activeDegreePlanId,
        type: "INTERNAL_NODE",
        children,
        progress,
    ...
    return (
        <div>
            <DegreeHeader
                ...
                skeleton={false}
            />
            <WhiteSpace $headerHeight={headerHeight} $zIndex={998} />
            {!collapsed && !editMode && (
                <>
                    <DegreeBody>
                        {degree &&
                            degree.rules.map((rule: any, index: number) => {
                                return (
                                    <RuleComponent
...
const ReqPanel = ({
    ...
    const isRequirementsPanelStep =
        tutorialModalKey === "requirements-panel-1" ||
        tutorialModalKey === "edit-requirements";
    ...
    useEffect(() => {
        ...
        reqPanelRef.current.style.zIndex = isRequirementsPanelStep ? "1002" : "";
    }, [tutorialModalKey, componentRefs]);

    const {
        data: activeDegreeplanDetail = null,
        isLoading: isLoadingDegrees,
    } = useSWR<DegreePlan>(
        activeDegreeplan
            ? `/api/degree/degreeplans/${activeDegreeplan.id}`
            : null,
    );
    ...
    return (
        <PanelContainer style={{ position: "relative" }} ref={reqPanelRef}>
            <PanelHeader>
                ...
                <PanelTopBarIconList>
                    <TutorialHighlight
                        $active={tutorialModalKey === "edit-requirements"}
                        ref={editReqRef}
                    >
                        <EditButton
                            editMode={editMode}
            ...
            {!activeDegreeplan ? (
                ...
                <>
                    {activeDegreeplanDetail && (
                        <ReqPanelBody>
                            <ReqContent>
                                {activeDegreeplanDetail.degrees.length == 0 &&
                                    !editMode && <EmptyPanel />}
                                {activeDegreeplanDetail.degrees.map(
                                    (degree) => (
                                        <Degree
                                            key={degree.id}
