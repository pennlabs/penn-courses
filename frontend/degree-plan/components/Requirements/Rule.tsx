 ...
 */
const RuleComponent = (
    ruleTree: RuleTree & { headerHeight?: number; zIndex?: number },
) => {
            ...
            collect: (monitor) => {
                if (monitor.getItemType() === ItemTypes.COURSE_IN_EXPAND) {
                    return {
                        isOver:
                            !!monitor.isOver() &&
                            rule.id === monitor.getItem().rule_id,
    ...
    return (
        <>
            ...
            <WhiteSpace
                $headerHeight={
                    myHeight + (headerHeight || 0) + HEADER_DEFAULT_BUFFER
                }
                $zIndex={zIndex}
            />
            {!collapsed && (
