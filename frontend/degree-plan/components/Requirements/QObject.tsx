...
const interpolate = <T,>(arr: T[], separator: T) =>
    arr.flatMap((elem, index) =>
        index < arr.length - 1 ? [elem, separator] : [elem],
    );

type ConditionKey =
    | "full_code"
    | "semester"
    | "attributes__code__in"
    | "department__code"
    | "full_code__startswith"
    | "code__gte"
    | "code__lte"
...
const SearchConditionInner = ({ q }: SearchConditionInnerProps) => {
    ...
    const conditions = q.clauses.filter(
        (clause) => clause.type === "LEAF",
    ) as Condition[];
    const compounds = q.clauses.filter(
        (clause) => clause.type === "OR" || clause.type === "AND",
    ) as Compound[];

    const display = [];
    const compoundCondition: Partial<Record<
        ConditionKey,
        any | undefined
    >> = {};
    ...
    if ("attributes__code__in" in compoundCondition) {
        display.push(
            <Attributes
                attributes={
                    compoundCondition["attributes__code__in"] as string[]
                }
            />,
        );
    }
    if (
        "department__code" in compoundCondition &&
        "code__gte" in compoundCondition &&
        "code__lte" in compoundCondition
    ) {
        display.push(
            <AttributeWrapper>
                <span>{compoundCondition["department__code"]}</span>{" "}
                {compoundCondition["code__gte"]}-
                {compoundCondition["code__lte"]}
            </AttributeWrapper>,
        );
    } else if (
        "department__code" in compoundCondition &&
        ...
        display.push(
            <AttributeWrapper>
                {compoundCondition["department__code"]}{" "}
                {compoundCondition["code__gte"]}-9999
            </AttributeWrapper>,
        );
    } else if (
        "department__code" in compoundCondition &&
        ...
        display.push(
            <AttributeWrapper>
                {compoundCondition["department__code"]} 0000-
                {compoundCondition["code__lte"]}
            </AttributeWrapper>,
        );
        ...
        display.push(
            <div>
                course number {compoundCondition["code__lte"]}-
                {compoundCondition["code__gte"]}
            </div>,
        );
    } else if ("code__lte" in compoundCondition) {
        display.push(
            <div>course number &lt;= {compoundCondition["code__lte"]}</div>,
        );
    } else if ("code__gte" in compoundCondition) {
        display.push(
            <div>course number &gt;= {compoundCondition["code__gte"]}</div>,
        );
    ...
    if ("department__code__in" in compoundCondition) {
        const departments = compoundCondition[
            "department__code__in"
        ] as string[];
        display.push(
            <Row>
    ...
    if ("full_code__startswith" in compoundCondition) {
        ...
        display.push(
            <div>
                {compoundCondition["full_code__startswith"]}
                {padding.slice(
                    (compoundCondition["full_code__startswith"] as string)
                        .length,
                )}
            </div>,
        );
...
const SearchCondition = ({
    ...
    return (
        <SearchConditionWrapper
            ...
            ref={ref}
        >
            {unselectedFulfillments.length > 0 ||
            (num &&
                fulfillments.length + unselectedFulfillments.length > num) ||
            (cu && fulfillments.length + unselectedFulfillments.length > cu) ? (
                <span
                    onClick={() => {
                        if (toExpand && canExpandAgain) {
                            setToExpand(false);
                ...
                >
                    <ExpandedCoursesPanelTrigger
                        searchRef={ref}
                        courses={unselectedFulfillments}
                        ruleId={ruleId}
                        changeExpandIcon={() => {
                            setToExpand(true);
                        }}
...
const transformCourseClauses = (q: ParsedQObj): ParsedQObj => {
    if (q.type === "LEAF" && q.key === "full_code")
        return { type: "COURSE", full_code: q.value as string };
    if (q.type === "AND" && q.clauses.length == 2) {
        const semester = (q.clauses.find(
            (clause) => clause.type === "LEAF" && clause.key === "semester",
        ) as Condition | undefined)?.value as string | undefined;
        const full_code = (q.clauses.find(
            (clause) => clause.type === "LEAF" && clause.key === "full_code",
        ) as Condition | undefined)?.value as string | undefined;
        if (full_code) return { type: "COURSE", semester, full_code };
    }
    // parse recursively
    if (q.type === "AND" || q.type === "OR")
        return { ...q, clauses: q.clauses.map(transformCourseClauses) };
    return q;
};
const transformDepartmentInClauses = (q: ParsedQObj): ParsedQObj => {
    if (q.type === "LEAF" && q.key === "department__code__in")
        return {
            type: "OR",
            clauses: (q.value as string[]).map((dept) => ({
                type: "LEAF",
                key: "department__code",
                value: dept,
            })),
        };
    if (q.type === "AND" || q.type === "OR")
        return { ...q, clauses: q.clauses.map(transformDepartmentInClauses) };
    return q;
};
const transformSearchConditions = (q: ParsedQObj): ParsedQObj => {
    ...
    const searchConditions = clauses.filter(
        (clause) => clause.type === "SEARCH",
    ) as Search[];
    ...
    if (searchConditions.length === 1) clauses.push(searchConditions[0]);
        ...
        clauses.push({
            ...
            q: {
                type: "OR",
                clauses: searchConditions.map(
                    (searchCondition) => searchCondition.q,
                ),
            },
        });
    ...
    switch (q.type) {
        case "OR":
            ...
            q.clauses.forEach((clause) => {
                if (clause.type === "COURSE" && clause.semester)
                    coursesWithSemester.push(clause);
                else if (clause.type === "COURSE") courses.push(clause);
                else if (clause.type === "SEARCH")
                    searchConditions.push(clause);
                else
                    throw Error(
                        `Non search or course clause in transformed Q object: ${JSON.stringify(
                            clause,
                        )}`,
                    );
            });
            ...
            const displayCoursesWithSemesters = coursesWithSemester.map(
                (course) => {
                    assert(typeof course.semester === "string");
                    const fulfillment = fulfillmentsMap.get(course.full_code);
                    const isChosen = !!(
                        fulfillment && fulfillment.semester === course.semester
                    );
                    ...
                    return (
                        <CourseInReq
                            course={{ ...course, rules: fulfillment?.rules }}
                            fulfillment={fulfillment}
                            isDisabled={satisfied && !isChosen}
                            isUsed={isChosen}
                            rule_id={rule.id}
                            activeDegreePlanId={activeDegreePlanId}
                        />
            ...
            assert(
                ...
                "Expected search conditions to be merged",
            ); // TODO!!
            const displaySearchConditions = searchConditions.map((search) => {
                ...
                return (
                    <SearchCondition
                        ...
                        unselectedFulfillments={unselectedFulfillments}
                        q={search.q}
            ...
            return (
                <Row $wrap>
                    {interpolate(
                        [
                            ...displayCoursesWithSemesters,
                            ...displayCoursesWithoutSemesters,
        ...
        case "COURSE":
            const [fulfillment] = fulfillments.filter(
                (fulfillment) =>
                    fulfillment.full_code == q.full_code &&
                    (!q.semester || q.semester === fulfillment.semester),
            );
            return (
                <CourseInReq
                    course={{
                        ...q,
                        rules: fulfillment ? fulfillment.rules : [],
                    }}
                    fulfillment={fulfillment}
                    isDisabled={satisfied && !fulfillment}
                    isUsed={!!fulfillment}
                    rule_id={rule.id}
                    activeDegreePlanId={activeDegreePlanId}
                />
    ...
    return (
        <RuleLeafWrapper $wrap>
            <QObject
                ...
                unselectedFulfillments={unselectedFulfillmentsForRule}
                rule={rule}
                satisfied={satisfied}
                activeDegreePlanId={activeDegreePlanId}
            />
