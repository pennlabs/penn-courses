...
import TutorialModal, {
    TutorialModalKey,
} from "./FourYearPlan/OnboardingTutorial";
...
const FourYearPlanPage = ({
    ...
    user,
    showTutorialModal,
}: FourYearPlanPageProps) => {
    const [tutorialModalKey, setTutorialModalKey] = useState<TutorialModalKey>(
        "welcome",
    );
    const semesterRefs = useRef<{ [semester: string]: HTMLDivElement | null }>(
        {},
    );
    ...
    const [
        expandedCoursesPanelCoords,
        setExpandedCoursesPanelCoords,
    ] = useState<{
        top?: number;
        left?: number;
        right?: number;
