...
export const ReviewPanelTrigger = ({
    full_code,
    triggerType,
    children,
}: PropsWithChildren<{
    ...
    const showReview = () => {
        if (!!open) {
            ...
            if (left > vw - right) position["right"] = vw - left;
            // set the right edge of the review panel to left edge of trigger
...
const ReviewPanel = ({
    ...
    const { data } = useSWR(`/api/base/all/courses/${full_code}`, {
        refreshInterval: 0,
