...
const InfoBanner = styled.div`
  ...
  background: ${props =>
    props.$isError
      ? "var(--pcr-color-feedback-error-bg)"
      : "var(--pcr-color-accent-subtle)"};
  border: 1px solid
    ${props =>
      props.$isError
        ? "var(--pcr-color-feedback-error-border)"
        : "var(--pcr-color-accent-subtle-border)"};
  color: ${props =>
    props.$isError
      ? "var(--pcr-color-feedback-error-accent)"
      : "var(--pcr-color-accent-hover)"};
  font-size: 14px;
...
const CourseResults = () => {
  ...
  const activeFilterCount = useMemo(() => countActiveFilters(filters), [
    filters
  ]);
  const activeSemesterFilters = useMemo(
    () => getActiveSemesterFilters(filters),
    [filters]
  );
  const isActivelyFiltering = activeFilterCount > 0;
  const formattedFilters = useMemo(() => formatFiltersForAPI(filters), [
    filters
  ]);
      ...
      {filteredResults !== null || isLoading ? (
              ...
              {isLoading ? (
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      Something went wrong while searching. Check your
                      connection and{" "}
                      <LinkText onClick={() => refetchSearch()}>
                        try again
                      </LinkText>
                      .
                    </span>
                ...
                <>
                  <SearchResultsHeader>
                    <span>
                      Showing <b>{Object.keys(filteredResults).length}</b> of{" "}
                      <b>{totalCount}</b> Search Results ({activeFilterCount}{" "}
                  ...
                  {activeSemesterFilters.length > 0 && (
                    <InfoBanner $isError={false}>
                      ...
                      <span>
                        Filtering by{" "}
                        {activeSemesterFilters
                          .map(k => SEMESTER_FILTER_LABELS[k])
                          .join(", ")}{" "}
                        — results limited to current semester offerings.
                      </span>
                    </InfoBanner>
                  )}
                ...
                >
                  ...
                  >
                    ...
                    {filters.semester === "Next Available" && (
                      <i>
                        Matching results may exist for previous semesters (Try
                        "All")
                      </i>
                    )}
                  </InfoBanner>
        ...
        <>
          ...
          <SubjectDisplayWrapper>
            {departments
              .slice(subjectSlice.start, subjectSlice.end)
              .map((dept, index) => (
